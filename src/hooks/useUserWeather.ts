...
import type { WeatherInfo } from "../utils/journal";
...
async function fetchLocationName(lat: number, lon: number, signal: AbortSignal): Promise<string> {
  try {
...
async function fetchWeather(lat: number, lon: number, signal: AbortSignal): Promise<WeatherInfo> {
  ...
  const locationName = await fetchLocationName(lat, lon, signal);
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          try {
            ...
            const nextWeather = await fetchWeather(lat, lon, controller.signal);
